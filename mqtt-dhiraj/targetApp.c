...
#define BROKER  ("ssl://b5feec6d24bf4ded89cbd15945207fb9.s2.eu.hivemq.cloud:8883")
#define KEEPALIVE (10)
...
int main(int argc, char** argv) 
{
  ...
  MQTTClient client;
  rc = MQTTClient_create(&client, BROKER, "Publisher", 
  ...
  rc = MQTTClient_subscribe(client, "Dhiraj/Level1", QOS);
  if (MQTTCLIENT_SUCCESS != rc)
  {
  ...
  while(!global_abort) 
  {
    ...
    {
      LOG(LOG_INFO, "publishing message %s to Dhiraj/Level1", rx_msg);
      // publish the message
      pubmsg.payload = rx_msg;
      pubmsg.payloadlen = bytes_read;
      pubmsg.qos = QOS;
      pubmsg.retained = 0;
      rc = MQTTClient_publishMessage(client, "Dhiraj/Level1", &pubmsg, NULL);
      if (MQTTCLIENT_SUCCESS != rc)
      {
